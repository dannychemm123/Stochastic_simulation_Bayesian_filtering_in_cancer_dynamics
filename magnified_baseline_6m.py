...
# Generate ODE Baseline Plot
print("Generating Fig_Magnified_ODE_Baseline_6m.png...")
...
for i, Ft in enumerate(F_targets):
    ...
    ax2 = ax.twinx()
    y0 = get_initial(Ft)
    sol = odeint(ode_func, y0, t_days, args=(0.0,)) # A_drug = 0.0
...
plt.savefig('Fig_Magnified_ODE_Baseline_6m.png', dpi=150)

# Generate SDE Baseline Plot
print("Generating Fig_Magnified_SDE_Baseline_6m.png...")
...
for i, Ft in enumerate(F_targets):
    ...
    for r in range(10):
        ...
        for td in t_days:
            while curr_t < td:
                d = np.array(ode_func(curr_x, curr_t, 0.0))
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig('Fig_Magnified_SDE_Baseline_6m.png', dpi=150)
print("✓ Saved separate ODE and SDE 6-month magnified baseline images.")
