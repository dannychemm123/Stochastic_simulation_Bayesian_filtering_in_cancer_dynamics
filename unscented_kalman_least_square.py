...
for _, row in clinical.iterrows():
    ...
    for t_idx, (t_months, t_days) in enumerate(zip(t_months_list, t_days_list)):
        ...
        if t_idx == 0:
            # Assimilate baseline CT measurement (no ODE propagation at t=0)
            x, P, innovation, S = ukf.update(x, P, ukf._sigma_points(x, P), obs_size)
            est_size  = cells_to_mm(x[0], x[1])
            unc       = ukf.obs_uncertainty_mm(x, P)

        else:
            # PREDICT: propagate sigma points through ODE
            x_pred, P_pred, sigmas_pred = ukf.predict(x, P)
            x_pred_size = cells_to_mm(x_pred[0], x_pred[1])

            # UPDATE: incorporate CT observation
...
# Save results
results_df.to_csv('UKF_Results_least_square.csv', index=False)
print("\n✓ UKF_Results_least_square.csv saved")
